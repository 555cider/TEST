...
# 지역(구) 클릭 시, 지역(구) 목록
@search.route("/category/district-gu", methods=["GET"])
def get_district_gu():
...
@search.route("/category/district-gu", methods=["POST"])
...
# 메뉴(메인) 클릭 시, 메뉴(메인) 목록
@search.route("/category/menu-main", methods=["GET"])
def get_menu_main():
...
@search.route("/category/menu-main", methods=["POST"])
def post_menu_main():
